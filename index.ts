...
function buildFocusPrompt(): string {
  return [
    "Focus tools are available: `focus_status`, `focus_join`, `focus_rejoin`, `focus_leave`, `focus_action`, `focus_clock`, `focus_noteboard_query`, `focus_noteboard_create`.",
    "Use them proactively for task lifecycle sync (start/switch/milestone/end) and timer/noteboard workflows, unless the user explicitly says not to sync.",
    "",
    "When to use `focus_action` (high priority):",
    "- Task start: before substantial work (reading/search/planning/editing), call `focus_action` once.",
    "- Task switch: when moving to a distinct task, call `focus_action` once.",
    "- Major milestone: after finishing a significant phase, call `focus_action` once.",
    ...
    "- Multi-step tasks: 3+ distinct operations (read/search/edit/test/validate).",
    "- Time-consuming operations likely taking 30+ seconds (builds, test suites, large scans).",
    "- User explicitly requests timer/pomodoro behavior.",
    "- At task start, estimate duration and prefer `countDown` mode.",
    "- Skip for trivial quick operations.",
    "",
    "Connection handling:",
    "- If Focus state is unknown, call `focus_status` first.",
    "- If `authKey` is missing, call `focus_join`.",
    "- If `authKey` exists but websocket is not open, call `focus_rejoin` (or wait for automatic rejoin on reconnect).",
    "",
    "How to choose `focus_action` parameters:",
    "- Choose poseType/action/bubble that match the current real activity.",
    "- Use only actions from the configured action list for that poseType.",
    "- bubble should be short (recommended 2-5 words).",
    "",
    "Skip Focus sync only when:",
    "- User explicitly asks not to sync.",
